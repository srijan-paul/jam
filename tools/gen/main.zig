...
pub fn main(init: std.process.Init) !void {
    ...
    var ast = (try parseAstZigSource(io, allocator))[0];
    defer ast.deinit(allocator);
    defer allocator.free(ast.source);

    var ast_types = try collectTypeNodesFromAst(allocator, ast);
    defer ast_types.struct_decls.deinit();

    {
        ...
        const out_file = try cwd.createFile(io, out_file_path, .{ .truncate = true });
        defer out_file.close(io);

        try generateNodeIterator(io, allocator, &ast_types, out_file);
    }
}

/// Writes zig code the represents the NodeIterator to the given file handle.
fn generateNodeIterator(io: std.Io, allocator: Allocator, types: *const TypesInAst, out_file: std.Io.File) !void {
    ...
    _ = try out_file.writeStreamingAll(io, iterator_file_start);

    var iter = types.struct_decls.iterator();
    while (iter.next()) |kv| {
        const k = kv.key_ptr.*;
        const v = kv.value_ptr.*;
        try generateVisitFn(io, allocator, out_file, types, k, v);
    }

    _ = try out_file.writeStreamingAll(io,
    ...
    _ = try out_file.writeStreamingAll(io, "}\n"); // close switch
    _ = try out_file.writeStreamingAll(io, "}"); // close fn
...
fn generateCaseArm(
    ...
    if (type_ann_node.tag == .optional_type) {
        ...
        if (std.mem.eql(u8, type_name, "Token")) {
            _ = try out_file.writeStreamingAll(io, ".");
            _ = try out_file.writeStreamingAll(io, field_name);
            _ = try out_file.writeStreamingAll(io, " => {}, // leaf (token)\n");
        } else if (std.mem.eql(u8, type_name, "Node")) {
            _ = try out_file.writeStreamingAll(io, ".");
            _ = try out_file.writeStreamingAll(io, field_name);
            if (is_optional_type) {
                _ = try out_file.writeStreamingAll(io, " => |maybe_child_id| if (maybe_child_id) |child_id| try self.pushNode(child_id, node_id),\n");
            } else {
                _ = try out_file.writeStreamingAll(io, " => |child_id| try self.pushNode(child_id, node_id),\n");
            }
        } else {
    ...
    if (type_ann_token == field_name_token) {
        _ = try out_file.writeStreamingAll(io, ".");
        _ = try out_file.writeStreamingAll(io, field_name);
        _ = try out_file.writeStreamingAll(io, " => {}, // leaf\n");
        return;
    }
    ...
    _ = try out_file.writeStreamingAll(io, ".");
    _ = try out_file.writeStreamingAll(io, field_name);
    _ = try out_file.writeStreamingAll(io, arm);
}

/// Parse the jam ast.zig file and return the AST of the ast (yeah...)
fn parseAstZigSource(io: std.Io, allocator: Allocator) !struct { zig.Ast, []u8 } {
    const ast_file_path = "src/js/ast.zig";

    var dir = std.Io.Dir.cwd();
    const ast_source = try dir.readFileAlloc(io, ast_file_path, allocator, std.Io.Limit.limited(std.math.maxInt(usize)));
    defer allocator.free(ast_source);

    const ast_source_z = try allocator.dupeZ(u8, ast_source);

    const ast = try zig.Ast.parse(allocator, ast_source_z, .zig);
    return .{ ast, ast_source_z };
}
...
fn generateStmtForFieldAccessAnn(
    ...
    if (std.mem.eql(u8, type_name, "Node")) {
        ...
        defer allocator.free(push_node_str);

        _ = try w.writeStreamingAll(io, push_node_str);
    }
}
...
fn generateStmtForAnn(
    ...
    if (type_ann_node.tag == .field_access) {
        ...
        _ = try w.writeStreamingAll(io, s);
...
fn generateVisitFn(
    ...
    _ = try w.writeStreamingAll(io, fn_header);
